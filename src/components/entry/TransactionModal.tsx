...
import { X, DollarSign, Bike, Car, Truck } from 'lucide-react';
import { AppType, TransactionType, TransactionInput } from '@/types';
...
export default function TransactionModal({ isOpen, onClose, onSubmit }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      trips: Number(trips),
      date,
      rider_name: 'Ice' // Will be overridden in page.tsx
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/60 backdrop-blur-md">
      <div className="bg-slate-900 w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[2.5rem] border-t sm:border border-slate-800 shadow-2xl overflow-hidden">
...
function AppOption({ active, onClick, label, icon, color }: AppOptionProps) {
  return (
